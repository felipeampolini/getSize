...
# Funcao que faz em formato de torta(pizza)
...
# Funcao que faz em formato de barras horizontais
def plot_hbars(sizes, names):
    plt.barh(names, sizes)
    plt.title('Tamanho dos diretorios')
    plt.ylabel('Diretorios')
    plt.xlabel('Tamanho')
    plt.show()

if __name__ == "__main__":
    folder_path = input("Digite o caminho do diretorio: ")
    ...
    print("[+] Tamanho total do diretorio", get_size_format(sum(directory_sizes)))

    # Chamada da função do codigo original
    # plot_pie(directory_sizes, names)

    # Chamada da função que gera em barras
    plot_hbars(directory_sizes, names)
